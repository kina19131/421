...
def planning(rrt_dubins, display_map=False):
   ...
   while i < rrt_dubins.max_iter:
      ...
      x = random.randint(rrt_dubins.x_lim[0], rrt_dubins.x_lim[1])
      y = random.randint(rrt_dubins.y_lim[0], rrt_dubins.y_lim[1])
      yaw = np.deg2rad(random.randint(-180, 180))

      rand_init_node = rrt_dubins.Node(x,y,yaw)
      ...
      for node in rrt_dubins.node_list:
         dist = rrt_dubins.calc_new_cost(rand_init_node, node)
         if dist < min_dist:
            nearest = node 
            min_dist = dist 

      new_node = rrt_dubins.propogate(nearest, rand_init_node) #example of usage
      ...
      if rrt_dubins.check_collision(new_node):
         ...
         # consider the 'current' new node as a midpoint, create another 'new node' 
         mid_point = new_node
         new_node = rrt_dubins.propogate(mid_point, rrt_dubins.goal)

         if rrt_dubins.check_collision(new_node):  
            #trace = new_node
            while new_node != None:
               ret_path.append(new_node)
